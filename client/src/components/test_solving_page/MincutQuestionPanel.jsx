...
import { cytoscapeConfig, stylesheetForMincut } from "../../lib/content";
import cl from "../../styles/TestsPage.module.css";

const MincutQuestionPanel = ({ question, setIsTestCompleted }) => {
    ...
    const renderTooltip = (props) => (
        <Tooltip {...props}>
            Зажмите Ctrl, чтобы выбрать несколько дуг или снять выделение c них
        </Tooltip>
    );

    const checkAnswerForMinCutQuestion = async () => {
        try {
            const response = await testSolvingStore.checkAnswerForMinCutQuestion(question.question_content.config, cyRefView.current.json().elements, cyRefInteraction.current.json().elements);
    ...
    return (
        <Row className='p-3 d-flex justify-content-between'>
            <Col className='color-2 main-border d-flex flex-column align-items-center rounded-4 p-3 me-2'>
                ...
                <p className='sub-font-reg text-black mb-0 ms-5'>Остаточная транспортная сеть</p>
            ...
            <Col className='color-2 main-border d-flex flex-column align-items-center rounded-4 p-3 ms-2'>
                ...
                <div id={cl.interactionNetworkPanel} className='w-100 d-flex justify-content-center align-items-center'
                    ...
                    <CytoscapeComponent
                        ...
                        style={cytoscapeConfig.style}
                        stylesheet={stylesheetForMincut}
                ...
                <div className='w-100 d-flex justify-content-end align-items-center'>
                    <button id={cl.sendAnswerButton} className='main-border color-3 rounded-4 px-5 py-2 main-font-bold fs-5'
                        onClick={checkAnswerForMinCutQuestion} >Ответить</button>
                </div>
            </Col>
        </Row>
    );
...
export default observer(MincutQuestionPanel);